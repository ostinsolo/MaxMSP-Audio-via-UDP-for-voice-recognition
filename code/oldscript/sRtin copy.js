...
const windowFunction = createHanningWindow(CHUNK_SIZE);
...
function convertMatrixToAudio(matrixData) {
  ...
  // Apply window function to the chunk
  const windowedData = new Float32Array(trimmedData.length);
  for (let i = 0; i < trimmedData.length; i++) {
    windowedData[i] = trimmedData[i] * windowFunction[i];
  }
  ...
  const rawMin = Math.min(...trimmedData);
  const rawMax = Math.max(...trimmedData);
  const rawAvg = trimmedData.reduce((sum, val) => sum + val, 0) / trimmedData.length;
...
function crossFadeChunks(prevChunk, currentChunk) {
  const fadeLength = OVERLAP_SAMPLES;
  for (let i = 0; i < fadeLength; i++) {
    const prevWeight = (fadeLength - i) / fadeLength;
    const currentWeight = i / fadeLength;
    currentChunk[i] = prevChunk[prevChunk.length - fadeLength + i] * prevWeight + currentChunk[i] * currentWeight;
  }
}
...
function createHanningWindow(length) {
  ...
  for (let i = 0; i < length; i++) {
    window[i] = (1 - Math.cos((2 * Math.PI * i) / (length - 1)));
  }
  return window;
}

function mergeChunks(prevChunk, currentChunk) {
  const fadeLength = OVERLAP_SAMPLES; // Length of the cross-fade region
  const fadeWindow = createHanningWindow(fadeLength); // Window function for cross-fade

  for (let i = 0; i < fadeLength; i++) {
    const prevSample = prevChunk[prevChunk.length - fadeLength + i];
    const currentSample = currentChunk[i];
    const fadeMultiplier = fadeWindow[i];

    // Apply cross-fade using a windowed interpolation
    currentChunk[i] = prevSample * fadeMultiplier + currentSample * (1 - fadeMultiplier);
  }
}
